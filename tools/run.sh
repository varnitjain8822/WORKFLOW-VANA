...
STAGES_DIR="$SCRIPT_DIR/../templates"
OUTPUT_DIR="$SCRIPT_DIR/../components"
INTERACTIVE=true
VERBOSE=false
...
generate_stage() {
    ...
    # NEW: Apply specialized skills for specific stages
    if [ "$stage_num" -eq 9 ] && [ -f "$SCRIPT_DIR/../skills/web-design/SKILL.md" ]; then
        print_info "🧠 Loading specialized 'web-design' skill into context..."
        echo -e "\n\n=========================================\n# SKILL MODULE: WEB DESIGN INSTRUCTIONS\nThe following are strict guidelines and design systems you MUST follow for UI/UX.\nHowever, DO NOT restrict yourself entirely to these instructions. You must also actively leverage your broad LLM knowledge, general best practices, and creative problem-solving skills to complete the task.\n=========================================\n" >> "$context_file"
        cat "$SCRIPT_DIR/../skills/web-design/SKILL.md" >> "$context_file"
    elif [[ "$stage_num" -eq 3 || "$stage_num" -eq 4 ]] && [ -f "$SCRIPT_DIR/../skills/system-architecture/SKILL.md" ]; then
        print_info "🧠 Loading specialized 'system-architecture' skill into context..."
        echo -e "\n\n=========================================\n# SKILL MODULE: SYSTEM ARCHITECTURE INSTRUCTIONS\nThe following are strict guidelines and architectural systems you MUST follow.\n=========================================\n" >> "$context_file"
        cat "$SCRIPT_DIR/../skills/system-architecture/SKILL.md" >> "$context_file"
    fi
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        if [ -n "$template_file" ]; then
            python3 "$SCRIPT_DIR/llm_client.py" --action generate --stage "$stage_num" --context-file "$context_file" --template-file "$template_file" --feedback "$feedback" --component "${COMPONENT_NAME:-Main System}" --input "$INPUT_FILE" > "$output_file"
        else
            python3 "$SCRIPT_DIR/llm_client.py" --action generate --stage "$stage_num" --context-file "$context_file" --feedback "$feedback" --component "${COMPONENT_NAME:-Main System}" --input "$INPUT_FILE" > "$output_file"
        fi
...
generate_all_stages() {
    ...
    while [ $i -le 9 ]; do
        ...
        if [ "$user_choice" -eq 1 ]; then
            ...
            if [ "$i" -eq 6 ]; then
                print_info "🔨 Extracting executable code files to the specific frontend and backend folders..."
                local project_output_root="$SCRIPT_DIR/.."
                mkdir -p "$project_output_root"
                python3 "$SCRIPT_DIR/extract_code.py" "$file_to_review" "$project_output_root"
            fi
